...
async function buildApp() {
  ...
  app.get("/api/config-public", (req, res) => {
    ...
    res.json({
      site: c.site || {},
      reseau: c.reseau || {},
      paymentMethods: (c.paymentMethods || []).filter((p) => p.enabled !== false)
    });
  });
  ...
  app.get("/api/packages", (req, res) => {
    const pkgs = (db.data.packages || []).filter((p) => p.active !== false);
    res.json({ packages: pkgs });
  });

  app.post("/api/orders", rateLimit("orders", 30, 60 * 1000), async (req, res) => {
    try {
      const order = payments.createOrder(db, req.body || {});
      res.json(order);
    } catch (e) {
