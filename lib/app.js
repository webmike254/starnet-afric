...
function createApp() {
  ...
  admin.post("/packages", async (req, res) => {
    ...
    if (db.data.packages.some((x) => x.code === code)) return res.status(400).json({ error: "Code déjà utilisé." });
    db.data.packages.push({
      code,
      nom: String(p.nom || code).slice(0, 80),
      quantiteGo: Number(p.quantiteGo) || 0,
      prix: Number(p.prix) || 0,
      prixPromo: Number(p.prixPromo) || 0,
      devise: String(p.devise || "CDF").slice(0, 6),
      type: p.type || "data",
      actif: p.actif !== false
    });
  ...
  admin.patch("/packages/:code", async (req, res) => {
  ...
  admin.get("/config", (req, res) => res.json({ config: db.data.config, admin: { user: db.data.admin.user } }));
  admin.patch("/config", async (req, res) => {
    if (req.body && req.body.site) Object.assign(db.data.config.site, req.body.site);
    if (req.body && req.body.reseau) Object.assign(db.data.config.reseau, req.body.reseau);
    try { await db.flushNow(); } catch (_) { }
    res.json({ ok: true });
  });
  ...
  admin.post("/password", async (req, res) => {
    ...
    if (!next || String(next).length < 6) return res.status(400).json({ error: "Nouveau mot de passe trop court." });
    const salt = crypto.randomBytes(16).toString("hex");
    db.data.admin.salt = salt;
    db.data.admin.passwordHash = security.hashPassword(String(next), salt);
    try { await db.flushNow(); } catch (_) { }
    res.json({ ok: true });
  });
  ...
  admin.post("/telegram/test", async (req, res) => {
    const r = await telegram.notify("🔔 Test notification Starnét Afric");
    res.json(r);
  });
