...
function createApp() {
  ...
  function rateLimit(key, max, windowMs) {
    return (req, res, next) => {
      ...
      if (b.ats.length >= max) return res.status(429).json({ error: "Trop de requêtes." });
      b.ats.push(now);
      next();
    };
  }
  ...
  app.post("/api/contact", rateLimit("contact", 5, 60000), async (req, res) => {
    ...
    telegram.notify("📩 <b>Nouveau message</b>\n👤 " + escHTML(String(nom).slice(0, 120)));
    res.json({ ok: true, id });
  });
  ...
  app.post("/api/orders", rateLimit("orders", 10, 10 * 60 * 1000), async (req, res) => {
    ...
    try { await db.flushNow(); } catch (e) {}
    telegram.notify("🛒 <b>Nouvelle commande</b> " + escHTML(String(reference)));
  ...
  admin.post("/telegram/test", async (req, res) => res.json(await telegram.notify("🔔 Test Starnét Afric")));
